...
### Superbook
The Superbook container is made up of HTML structure as shown above with each `<page>` a virgin `iframe` that is rendered independently from others in either dual or singular mode responsively and automatically. Each `<page>` contains a sourcedoc i.e. content that is rendered in real-time and is sandboxed from the rest of the book like a typical *single page app* (SPA). The SPA itself is `offline-first` using a serviceworker to make way for offline reading.
