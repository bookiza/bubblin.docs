...
published: true
---
In this tutorial we'll learn about writing a Superbook on web.

#### Using the web editor

Bubblin allows book writing over the web using its in-browser [book editor]({{ site.baseurl }}{% post_url 2018-01-01-setup %})—a simple code playground that is great for publishing short immersive stories, comics, novellas, weekly or biweekly magazines and other stuff like brochures, greeting cards, restaurant menus etc. Theoretically speaking one could write a full sized textbook using the online book editor on Bubblin but it is generally not recommended. For really long body of works we recommend using [Bookiza](http://bookiza.io)—a professional book baking tool—instead.


The following tutorial will focus on using the in-browser book editor:
...
<br/>
<br/>
A new project will look something like this: with three large gray colored textareas at the bottom to punch in HTML, CSS and JavaScript on each page. It will also contain four bluish colored empty pages (two leafs) to start your story with.
